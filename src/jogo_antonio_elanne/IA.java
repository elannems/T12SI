    ...
    public int evaluateBoard(Tabuleiro b){
        ...
        for(int i=5;i>=0;--i){
            for(int j=0;j<=6;++j){
                ...
                if(b.getTabuleiro()[i][j]==0 || b.getTabuleiro()[i][j]==2) continue; 
                
                if(j<=3){ 
                    for(k=1;k<4;++k){
                        if(b.getTabuleiro()[i][j+k]==1)aiScore++;
                        else if(b.getTabuleiro()[i][j+k]==2){aiScore=0;blanks = 0;break;}
                        else blanks++;
                    }
                    ...
                    if(blanks>0) 
                        for(int c=1;c<4;++c){
                            int column = j+c;
                            for(int m=i; m<= 5;m++){
                             if(b.getTabuleiro()[m][column]==0)moreMoves++;
                                else break;
                            } 
                        } 
                ...
                if(i>=3){
                    for(k=1;k<4;++k){
                        ...
                        else if(b.getTabuleiro()[i-k][j]==2){aiScore=0;break;} 
                    } 
                    moreMoves = 0; 
                ...
                if(j<=3 && i>=3){
                    for(k=1;k<4;++k){
                        ...
                        else if(b.getTabuleiro()[i-k][j+k]==2){aiScore=0;blanks=0;break;}
                        else blanks++;                        
                    }
                    moreMoves=0;
                    if(blanks>0){
